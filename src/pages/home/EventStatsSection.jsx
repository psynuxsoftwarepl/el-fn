...
const EventStatsSection = () => {
  ...
  return (
    <HeroSection className="bg-[url('/assets/eleqt-events.jpg')] bg-center bg-cover">
      ...
      <div className="relative z-10 max-w-6xl mx-auto text-center px-4 py-6 flex flex-col items-center w-full min-h-screen overflow-y-auto justify-start">
        ...
        <div className="mb-4 text-center">
          <img
            src="/assets/Eleqt Transparent logo.svg"
            alt="Eleqt Logo"
            className="h-12 md:h-20 w-auto mx-auto mb-1"
          />
          <p className="uppercase text-white text-sm md:text-xl tracking-widest font-bold">
            FOR EVENTS
          </p>
        </div>

        {/* === STATS GRID (2 CARDS ONLY) === */}
        <div className="mb-6 w-full max-w-3xl grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            { value: "05+", label: "Weddings Served" },
            { value: "15+", label: "Venue Collaborations" },
          ].map((item, index) => (
            <div
              key={index}
              className="border border-[#B8A171] rounded-md py-6 px-6 bg-[#111111] bg-opacity-30 backdrop-blur-md flex flex-col items-center"
            >
              <h3 className="text-4xl font-bold text-[#B8A171]">{item.value}</h3>
        ...
        <div>
          <button
            onClick={() => navigate("/event-experience")}
            className="border border-[#B8A171] text-white hover:bg-gradient-to-r from-[#B8A171] to-[#FFF2CC] hover:text-black transition px-6 py-2 rounded-full text-sm md:text-base font-medium"
          >
            Explore More
          </button>
        </div>
